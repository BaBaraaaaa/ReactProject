...
import { subCategories, filters, color, singleFilter } from "./FilterData.js";
import {
  FormControl,
  FormLabel,
  FormControlLabel,
...
const navigate = useNavigate();
  ...
  const handleFilter= (value,sectionId) =>{
   const searchParamms = new URLSearchParams(location.search)
    ...
    //kiểm tra value  đã tồn tại trong sectionId của query parameter chưa
    if(filterValue.length>0 && filterValue[0].split(",").includes(value))
    {
      //nếu rồi thì loại bỏ value trong filtervalue 
      filterValue = filterValue[0].split(",").filter((item)=>item!==value);
      //nếu filtervalue sao khi loại bỏ mà trở thành rỗng thì sẽ xóa luôn sectionid khỏi 
      //searchParam 
